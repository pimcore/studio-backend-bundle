...
use Pimcore\Bundle\StudioBackendBundle\Grid\Service\Loader\TaggedIteratorPhpCodeTransformerLoader;
 ...
 * @internal
 */
final readonly class PhpCodeTransformerPass implements CompilerPassInterface
{
    ...
    public function process(ContainerBuilder $container): void
    {
        $taggedServices = array_keys(
            [
                ... $container->findTaggedServiceIds(TaggedIteratorPhpCodeTransformerLoader::PHPCODE_TRANSFORMER_TAG),

            ]
        );

        foreach ($taggedServices as $environmentType) {
            $this->checkInterface($environmentType, PhpCodeTransformerInterface::class);
        }
    }
}

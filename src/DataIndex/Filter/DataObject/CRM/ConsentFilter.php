...
namespace Pimcore\Bundle\StudioBackendBundle\DataIndex\Filter\DataObject\CRM;
 ...
 * @internal
 */
final class ConsentFilter implements FilterInterface
{
    public function apply(mixed $parameters, QueryInterface $query): QueryInterface
    {
        ...
        foreach ($parameters->getColumnFilterByType(ColumnType::CRM_CONSENT->value) as $column) {
            if (!is_array($column->getFilterValue())) {
                throw new InvalidArgumentException('Filter value for this filter must be a array');
            }

            $query->booleanFilter($column->getKey().'.consent', $column->getFilterValue());
        }

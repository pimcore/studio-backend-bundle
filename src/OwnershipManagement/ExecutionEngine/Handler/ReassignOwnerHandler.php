 ...
 */
#[AsMessageHandler]
final class ReassignOwnerHandler extends AbstractHandler
{
     ...
     * @throws Exception
     */
    public function __invoke(ReassignOwnerMessage $message): void
    {
        ...
        $newOwnerId = (int) $this->extractConfigFieldFromJobStepConfig($message, StepConfig::NEW_OWNER_ID->value);
        ...
        foreach ($ids as $id) {
            try {
                $provider->reassignOwner([(string) $id], $newOwnerId);
            } catch (Exception $e) {
                $this->abort($this->getAbortData(
                    Config::OWNERSHIP_MANAGEMENT_REASSIGN_FAILED->value,

...
`studio_mcp_login` is not a request limiter - it counts failed authentication attempts on the MCP firewall and is driven by the security layer rather than by the general subscriber. See [MCP Server](../04_Development_Details/08_MCP_Server.md#throttling-guessed-credentials) for how it is keyed and why a successful authentication is never affected by it.
...
This switches off both `studio_api_general` and `studio_mcp_general`, and with them the `X-RateLimit-*` headers. It does **not** affect `studio_mcp_login`, which lives on the MCP firewall rather than in this subscriber - remove `login_throttling` from `pimcore_studio_backend.mcp_firewall_settings` to turn that off.
